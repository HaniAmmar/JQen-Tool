...
a:ha},Y=await (async function(){function a(d){Y=d.exports;F=Y.f;L();d=Y;c._JQenRender=d.h;na=d.i;la=d.j;X=d.k;ka=d.l;M--;c.monitorRunDependencies?.(M);0==M&&N&&(d=N,N=null,d());return Y}M++;c.monitorRunDependencies?.(M);var b={a:oa};if(c.instantiateWasm)return new Promise(d=>{c.instantiateWasm(b,(f,h)=>{d(a(f,h))})});P??=c.locateFile?c.locateFile("JQen.wasm",v):v+"JQen.wasm";return a((await ea(b)).instance)}());
